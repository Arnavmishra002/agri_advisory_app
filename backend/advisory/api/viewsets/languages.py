...
class LanguageViewSet(viewsets.ViewSet):
    ...
    def detect(self, request):
        ...
        if not serializer.is_valid():
            return Response({"error": "Invalid language detection parameters", "errors": serializer.errors}, status=400)
    ...
    def normalise(self, request):
        ...
        serializer = LocationQuerySerializer(data=request.query_params)
        if not serializer.is_valid():
            return Response({"error": "Invalid language parameters", "errors": serializer.errors}, status=400)
        raw = serializer.validated_data.get("lang", "hi")
