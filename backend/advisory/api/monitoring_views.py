...
def _health_timestamp() -> str:
    """ISO timestamp for health endpoints; safe when metrics collection fails."""
    try:
        status = performance_monitor.get_system_health_status()
        return status.get("timestamp") or datetime.now().isoformat()
    except Exception:
...
def liveness_check(request):
    ...
    try:
        ...
        return JsonResponse({
            ...
            'timestamp': system_health.get('timestamp') or _health_timestamp()
        })
    except Exception as e:
