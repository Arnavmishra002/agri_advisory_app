...
class CleanWeatherAPI:
    ...
    def get_current_weather(self, location: str, latitude: float = None, longitude: float = None) -> Dict[str, Any]:
        """Current conditions for pest/region verification (KrishiRaksha)."""
        data = self.get_weather_data(location, latitude, longitude)
        if data.get("current"):
            return data["current"]
    ...
    def _try_open_meteo_api(self, latitude: float, longitude: float, location: str) -> Optional[Dict[str, Any]]:
        """Try Open-Meteo API — uses the full 'current' parameter set to get real humidity."""
        try:
            # Use 'current' (not deprecated 'current_weather') to get humidity + full fields
            url = (
    ...
    def _get_enhanced_fallback_data(self, location: str) -> Dict[str, Any]:
        """
        Honest fallback — all real-time APIs failed.
        Returns a clearly labeled response with is_live=False.
        No random values are presented as real weather data.
        """
        return {
            'status': 'fallback',
            'is_live': False,
            'data': {
                ...
                'location': location,
                'timestamp': datetime.now().isoformat(),
                'data_source': 'All weather APIs unavailable — check OPENWEATHER_API_KEY in .env',
            },
            'forecast_7_days': [],
            'agricultural_advice': 'वास्तविक समय मौसम डेटा अनुपलब्ध — कृपया IMD देखें: mausam.imd.gov.in',
            'data_source': 'Fallback (all APIs unavailable)',
            'timestamp': datetime.now().isoformat(),
            'message': (
                'Real-time weather unavailable. '
                'Set OPENWEATHER_API_KEY in .env for live data (https://openweathermap.org/api). '
                'Open-Meteo is free and requires no key — check network connectivity.'
            ),
        }
    
    def _get_7day_forecast(self, location: str) -> list:
        """
        Returns an empty forecast list — callers should use unified_realtime_service
        which fetches a real 7-day forecast from Open-Meteo.
        This method exists only as a safety stub.
        """
        return []
