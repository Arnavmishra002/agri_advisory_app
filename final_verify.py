...
def test_weather_service():
    ...
    # Test the correct weather endpoint with GET method
    result = test_api_endpoint("/api/realtime-gov/weather/?location=Delhi&latitude=28.7041&longitude=77.1025", "GET", description="Weather Data")
    ...
    if result["status"] == "SUCCESS":
        ...
        if "weather_data" in data:
            ...
            print(f"Wind Speed: {weather.get('wind_speed', 'N/A')}")
        ...
        if "forecast" in data and data["forecast"]:
            ...
            for i, day in enumerate(data["forecast"][:3]):
                print(f"   Day {i+1}: {day.get('temperature', 'N/A')} - {day.get('condition', 'N/A')}")
    else:
...
def test_market_prices_service():
    ...
    # Test the correct market prices endpoint with GET method
    result = test_api_endpoint("/api/realtime-gov/market_prices/?commodity=wheat&state=Delhi", "GET", description="Market Prices")
    ...
    if result["status"] == "SUCCESS":
        ...
        if "price_trends" in data and data["price_trends"]:
            ...
            for trend in data["price_trends"][:3]:
                print(f"   {trend.get('date', 'N/A')}: {trend.get('price', 'N/A')}")
    else:
...
def test_crop_recommendations_service():
    ...
    # Test the correct crop recommendations endpoint with GET method
    result = test_api_endpoint("/api/realtime-gov/crop_recommendations/?location=Delhi&season=rabi", "GET", description="Crop Recommendations")
    ...
    if result["status"] == "SUCCESS":
        ...
        if "crop_recommendations" in data:
            ...
            for i, crop in enumerate(crops[:5]):
                ...
                print(f"      Profit: {crop.get('profit_per_acre', 'N/A')}")
                print(f"      Yield: {crop.get('yield_per_acre', 'N/A')}")
...
def test_government_schemes_service():
    ...
    # Test the correct government schemes endpoint with GET method
    result = test_api_endpoint("/api/realtime-gov/government_schemes/?location=Delhi", "GET", description="Government Schemes")
    ...
    if result["status"] == "SUCCESS":
        ...
        if "schemes" in data:
            ...
            for i, scheme in enumerate(schemes[:5]):
                ...
                print(f"      Amount: {scheme.get('amount', 'N/A')}")
...
def test_soil_health_service():
    ...
    # Test the correct soil health endpoint with GET method
    result = test_api_endpoint("/api/realtime-gov/soil_health/?location=Delhi", "GET", description="Soil Health")
...
def test_pest_detection_service():
    ...
    # Test the correct pest detection endpoint with GET method
    result = test_api_endpoint("/api/realtime-gov/pest_detection/?location=Delhi&crop=wheat", "GET", description="Pest Detection")
...
def test_ai_chatbot_service():
    ...
    # Test the correct AI chatbot endpoint with POST method
