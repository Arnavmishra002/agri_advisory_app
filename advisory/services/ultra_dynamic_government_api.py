...
class UltraDynamicGovernmentAPI:
    ...
    def get_weather_data(self, location: str, latitude: float = None, longitude: float = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if latitude and longitude:
                return self._fetch_weather_data(latitude, longitude, location)
            else:
    ...
    def _try_imd_api(self, latitude: float, longitude: float, location: str) -> Optional[Dict[str, Any]]:
        """Try IMD API for real-time weather"""
        try:
            # IMD API endpoints
            imd_urls = [
                f"https://mausam.imd.gov.in/api/weather?lat={latitude}&lon={longitude}",
                f"https://mausam.imd.gov.in/imd_latest/contents/surface_weather.php?lat={latitude}&lon={longitude}",
                f"https://mausam.imd.gov.in/imd_latest/contents/district_forecast.php?lat={latitude}&lon={longitude}"
            ]
            ...
            for url in imd_urls:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        return {
                            ...
                            'data': {
                                ...
                                'description': data.get('condition', 'साफ आसमान'),
                                'feels_like': f"{data.get('temperature', 28) + 2}°C",
                                'pressure': str(data.get('pressure', 1013)),
                                'pressure_unit': 'hPa',
                                'visibility': f"{data.get('visibility', 10)}",
                                'visibility_unit': 'km',
                                'uv_index': str(data.get('uv_index', 5)),
                                'rainfall_probability': data.get('rainfall_probability', 20),
                                'forecast_7day': data.get('forecast', []),
                                'farmer_advisory': data.get('advisory', 'मौसम अनुकूल है, नियमित खेती के कार्य जारी रखें'),
                    ...
                    logger.warning(f"IMD API URL {url} failed: {e}")
                    continue
    ...
    def _get_fallback_weather_data(self, location: str) -> Dict[str, Any]:
        """Enhanced fallback weather data with real-time simulation for all Indian locations"""
        # Directly use comprehensive location weather for dynamic data
